...
function updateProfile(data){
    ...
    // if data show not found if we enter wrong name
    if(data.message !== "Not Found") {
        // for showing not available function
        function checkNull(apiItem, domItem){
            if(apiItem === "" || apiItem == null){
                domItem.style.opacity = 0.7;
                domItem.previousElementSibling.style.opacity = 0.7;
                return false;
            } else{
                return true;
            }
...
getUserData(url + "adityadwivedi2002")